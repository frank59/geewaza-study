...
public class Request {
	...
	public static String requestGet(String url, String host, int soTimeout,
			int connTimeout, String proxyHost, int proxyPort) throws Exception {
		HttpClient client = new HttpClient();
		String result = null;
		GetMethod method = new GetMethod(url);
		...
		method.setRequestHeader("Cache-Control", "no-cache");
		method.setRequestHeader("host", "no-cache");
		...
		for (int i = 0; i < 3; i++) {
			try {
				...
				logger.error("第" + (i + 1) + "次请求失败.e.getMessage() = "
						+ e.getMessage() + ".soTimeout:" + soTimeout
						+ ",connTimeout:" + connTimeout + ",url:" + url
						+ ".使用的代理是：" + proxyHost + ":" + proxyPort);
				if (i < 2) {
	...
	public static String requestGet(String url, int soTimeout, int connTimeout,
	...
	public static byte[] requestGetByteArray(String url, int soTimeout,
			int connTimeout, String proxyHost, int proxyPort) throws Exception {
		HttpClient client = new HttpClient();
		byte[] result = null;
		GetMethod method = new GetMethod(url);
